      ...
      <div>
        ...
        {draft && (
          <form onSubmit={(event) => { event.preventDefault(); save(); }} className="mb-4 grid gap-3 rounded-2xl border border-[#f0c4ca] bg-white p-4">
            ...
            <label><span className="mb-1 block text-xs font-bold text-slate-600">المنطقة</span><input list="zone-options" value={draft.zone} onChange={(event) => setDraft({ ...draft, zone: event.target.value })} className={inputClass} /></label>
            <datalist id="zone-options">{zones.map((zone) => <option key={zone} value={zone} />)}</datalist>
            <label><span className="mb-1 block text-xs font-bold text-slate-600">أسماء بديلة (مفصولة بفاصلة)</span><textarea value={draft.aliases} onChange={(event) => setDraft({ ...draft, aliases: event.target.value })} rows={2} className="w-full rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-[#e6a1aa]" /></label>
            <p className="flex items-center gap-2 rounded-xl bg-blue-50 p-3 text-xs font-bold text-blue-800"><MapPin className="h-4 w-4 shrink-0" /> الموقع (انقر على الخريطة) · <span dir="ltr">{draft.lat.toFixed(5)}, {draft.lng.toFixed(5)}</span></p>
