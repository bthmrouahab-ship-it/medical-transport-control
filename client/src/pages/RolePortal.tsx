...
type Role = "clinic" | "supervisor" | "driver";
...
function RoleIcon({ role }: { role: Role }) {
  ...
  if (role === "supervisor") return <ShieldCheck className="h-5 w-5" />;
...
function RoleShell({ session, onLogout, onManager }: { session: Session; onLogout: () => void; onManager: () => void }) {
  ...
  const [view, setView] = useState<"home" | "new" | "notifications">("home");
  const isClinic = session.role === "clinic";
  const isSupervisor = session.role === "supervisor";
  const title = isClinic ? "مواعيد العيادة" : isSupervisor ? "طلبات السيارات" : "رحلاتي والتنبيهات";
  ...
  return <div className="min-h-screen bg-[#f5f7fb]" dir="rtl"><header className="sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-slate-200 bg-[#f5f7fb]/95 px-5 backdrop-blur-xl lg:px-10"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#10233f] text-[#4de0bc]"><RoleIcon role={session.role} /></div><div><p className="text-xs font-semibold text-[#1f7c76]">{isClinic ? "مساحة العيادة" : isSupervisor ? "مساحة الإشراف" : "مساحة السائق"}</p><h1 className="text-xl font-bold">{title}</h1></div></div><div className="flex items-center gap-2"><span className="hidden text-xs font-semibold text-slate-400 sm:inline">{session.name}</span>{isSupervisor && <button onClick={onManager} className="hidden rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 lg:inline">لوحة الأسطول</button>}<button aria-label="تسجيل الخروج" onClick={onLogout} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:text-red-600"><LogOut className="h-4 w-4" /></button></div></header><main className="mx-auto max-w-6xl p-5 lg:p-10">{view === "home" && isClinic && <ClinicHome appointments={appointments} onNew={() => setView("new")} />}{view === "new" && isClinic && <ClinicForm onBack={() => setView("home")} onSave={(appointment) => { updateAppointments([...appointments, appointment].sort((a, b) => a.appointmentAt.localeCompare(b.appointmentAt))); setView("home"); toast.success("تم تسجيل الموعد وأصبح متاحًا للمشرف"); }} />}{view === "home" && isSupervisor && <SupervisorHome appointments={appointments} requests={requests} onRequest={(request, appointmentId) => { updateRequests([...requests, request]); updateAppointments(appointments.map((a) => a.id === appointmentId ? { ...a, status: "تم طلب السيارة" } : a)); toast.success(`تم إنشاء طلب السيارة ${request.vehiclePlate}`); }} />}{view === "home" && session.role === "driver" && <DriverHome appointments={appointments} requests={requests} onNotify={() => setView("notifications")} />}{view === "notifications" && session.role === "driver" && <DriverNotifications appointments={appointments} requests={requests} onBack={() => setView("home")} />}</main></div>;
}
...
function SupervisorHome({ appointments, requests, onRequest }: { appointments: ClinicAppointment[]; requests: VehicleRequest[]; onRequest: (request: VehicleRequest, appointmentId: string) => void }) { const pending = appointments.filter((a) => a.status !== "مكتملة"); return <><PageHeading eyebrow="صلاحية طلب السيارة" title="اختر موعدًا ثم اطلب السيارة" description="لا يمكن طلب سيارة بدون موعد طبي مسجل من العيادة. بعد الطلب يظهر تنبيه للسائق." /><div className="mb-6 flex items-center gap-3 rounded-2xl border border-[#b7dfd7] bg-[#effbf8] p-4 text-sm text-[#165e5a]"><ShieldCheck className="h-5 w-5 shrink-0" /><span>قاعدة النظام: كل طلب مرتبط برقم موعد <b>APT</b> ولا يمكن إنشاؤه من شاشة فارغة.</span></div><div className="grid gap-4">{pending.map((appointment) => { const request = requests.find((r) => r.appointmentId === appointment.id); return <SupervisorAppointment key={appointment.id} appointment={appointment} request={request} onRequest={onRequest} />; })}</div></>; }

function SupervisorAppointment({ appointment, request, onRequest }: { appointment: ClinicAppointment; request?: VehicleRequest; onRequest: (request: VehicleRequest, appointmentId: string) => void }) { const [method, setMethod] = useState<"whatsapp" | "call">("whatsapp"); const vehicle = assignVehicle(vehicles, appointment.kind); function requestCar() { if (!canRequestVehicle(appointment, request) || !vehicle) { toast.error("لا يوجد موعد قابل للطلب أو لا توجد سيارة متاحة"); return; } onRequest({ id: `REQ-${Date.now()}`, appointmentId: appointment.id, vehiclePlate: vehicle.plate, driver: vehicle.driver, status: "مطلوب", notificationMethod: method, createdAt: new Date().toLocaleTimeString("ar-SA", { hour: "2-digit", minute: "2-digit" }) }, appointment.id); } return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_30px_rgba(25,45,85,.04)]"><div className="flex flex-col gap-4 lg:flex-row lg:items-center"><div className="flex flex-1 items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#eef4f7] text-[#1f5c66]"><CalendarDays className="h-5 w-5" /></div><div><div className="flex flex-wrap items-center gap-2"><p className="font-bold">{appointment.patientName}</p><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-500">{appointment.id}</span></div><p className="mt-1 text-xs text-slate-400">{appointment.appointmentAt} · {appointment.pickupArea} إلى {appointment.clinic} · {appointment.kind}</p></div></div><div className="flex items-center gap-2 text-xs text-slate-500"><Truck className="h-4 w-4 text-slate-300" />{request ? `${request.vehiclePlate} · ${request.driver}` : vehicle ? `مقترح: ${vehicle.plate} · ${vehicle.driver}` : "لا توجد سيارة"}</div><span className={`rounded-full px-3 py-1.5 text-xs font-bold ${request ? "bg-violet-50 text-violet-700" : "bg-amber-50 text-amber-700"}`}>{request ? "تم إنشاء الطلب" : "بانتظار الطلب"}</span>{!request && <div className="flex items-center gap-2"><select value={method} onChange={(e) => setMethod(e.target.value as "whatsapp" | "call")} className="h-10 rounded-xl border border-slate-200 bg-white px-2 text-xs font-bold"><option value="whatsapp">واتساب</option><option value="call">اتصال</option></select><button onClick={requestCar} className="flex min-h-10 items-center gap-2 rounded-xl bg-[#1f5c66] px-3 text-xs font-bold text-white hover:bg-[#164d57]"><BellRing className="h-4 w-4" /> طلب السيارة</button></div>}</div></div>; }

function DriverHome({ appointments, requests, onNotify }: { appointments: ClinicAppointment[]; requests: VehicleRequest[]; onNotify: () => void }) { const myRequests = requests.filter((r) => r.driver === "كمال"); const active = myRequests.map((r) => ({ request: r, appointment: appointments.find((a) => a.id === r.appointmentId) })).filter((x) => x.appointment); return <><PageHeading eyebrow="تنبيهات الرحلات" title="رحلاتي اليوم" description="ستظهر هنا الرحلات التي طلبها المشرف، مع قناة التنبيه المختارة." action={<button onClick={onNotify} className="flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-600"><BellRing className="h-4 w-4 text-[#1f7c76]" /> مركز التنبيهات</button>} />{active.length === 0 ? <EmptyState /> : <div className="grid gap-4">{active.map(({ request, appointment }) => <div key={request.id} className="rounded-2xl border border-[#b7dfd7] bg-white p-5 shadow-[0_8px_30px_rgba(25,45,85,.04)]"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold text-[#1f7c76]">رحلة جديدة · {request.id}</p><h3 className="mt-2 text-lg font-bold">استلام {appointment!.patientName}</h3><p className="mt-1 text-sm text-slate-500">{appointment!.pickupArea} إلى {appointment!.clinic} · الساعة {appointment!.appointmentAt}</p></div><span className="rounded-full bg-amber-50 px-3 py-1.5 text-xs font-bold text-amber-700">تنبيه مطلوب</span></div><div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4 text-xs"><span className="flex items-center gap-2 text-slate-500"><Truck className="h-4 w-4" /> السيارة {request.vehiclePlate}</span><span className="rounded-lg bg-slate-50 px-3 py-2 font-bold text-slate-600">القناة: {request.notificationMethod === "whatsapp" ? "واتساب" : "اتصال"}</span></div></div>)}</div>}</>; }

function DriverNotifications({ appointments, requests, onBack }: { appointments: ClinicAppointment[]; requests: VehicleRequest[]; onBack: () => void }) { const myRequests = requests.filter((r) => r.driver === "كمال"); return <><PageHeading eyebrow="مركز الإشعارات" title="تنبيهات السائق" description="هذه الأزرار تمثل نقطة التكامل مع مزود الاتصال أو واتساب الرسمي." action={<button onClick={onBack} className="flex items-center gap-2 text-sm font-bold text-[#1f7c76]"><ArrowLeft className="h-4 w-4" /> العودة للرحلات</button>} /><div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-800"><b>الإرسال الخارجي غير مفعّل بعد.</b> يحتاج النظام إلى ربط مزود واتساب رسمي أو خدمة اتصال مثل Twilio، ثم يمكن جعل التنبيه تلقائيًا عند تأكيد طلب السيارة.</div><div className="grid gap-4">{myRequests.map((request) => { const appointment = appointments.find((a) => a.id === request.appointmentId); if (!appointment) return null; const message = buildDriverMessage(appointment, request); return <div key={request.id} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#eefaf7] text-[#1f7c76]"><BellRing className="h-5 w-5" /></div><div><p className="font-bold">{request.id} · السيارة {request.vehiclePlate}</p><p className="mt-1 text-xs text-slate-400">{appointment.patientName} · {appointment.appointmentAt}</p></div></div><div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs leading-6 text-slate-600">{message}</div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => toast.info("سيتم إرسال رسالة واتساب بعد ربط WhatsApp Business API")} className="flex min-h-10 items-center gap-2 rounded-xl bg-[#168c65] px-3 text-xs font-bold text-white"><MessageCircle className="h-4 w-4" /> إرسال واتساب</button><button onClick={() => toast.info("سيبدأ الاتصال التلقائي بعد ربط مزود الاتصالات") } className="flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-xs font-bold text-slate-600"><PhoneCall className="h-4 w-4" /> إجراء اتصال</button></div></div>; })}</div></>; }
