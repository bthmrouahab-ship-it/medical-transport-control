    ...
    <div className="min-h-screen bg-[#f5f7fb] text-slate-900" dir="rtl">
        ...
        <div className="flex h-full flex-col px-4 py-5">
          ...
          <nav className="mt-3 space-y-1">
            {nav.map((item) => {
              ...
              return <button key={item.id} aria-current={active ? "page" : undefined} onClick={() => { setView(item.id); setMobileMenu(false); }} className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${active ? "bg-[#244669] text-[#83f0d0] shadow-inner" : "text-slate-300 hover:bg-white/5 hover:text-white"}`}><Icon className="h-[18px] w-[18px]" />{item.label}{item.id === "tracking" && <span className="mr-auto h-2 w-2 rounded-full bg-[#4de0bc]" />}</button>;
            })}
          </nav>
      ...
      <div className="min-h-screen lg:mr-[274px]">
        <header className="sticky top-0 z-30 flex h-[76px] items-center justify-between border-b border-slate-200/80 bg-[#f5f7fb]/90 px-5 backdrop-blur-xl lg:px-8">
          <div className="flex items-center gap-3"><button aria-label="فتح القائمة" className="min-h-11 min-w-11 rounded-xl border border-slate-200 bg-white p-2 lg:hidden" onClick={() => setMobileMenu(true)}><Menu className="h-5 w-5" /></button><div><div className="flex items-center gap-2"><span className="text-xs font-semibold text-slate-400">الاثنين، 22 سبتمبر 2026</span><span className="h-1 w-1 rounded-full bg-slate-300" /><span className="text-xs font-semibold text-emerald-600">وردية الصباح</span></div><h1 className="mt-1 text-xl font-bold text-slate-900">{currentTitle}</h1></div></div>
          <div className="flex items-center gap-2 sm:gap-3"><div className="hidden items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500 sm:flex"><Activity className="h-4 w-4 text-emerald-500" /> 18 سيارة على الطريق</div><button onClick={() => setShowNew(true)} className="flex min-h-11 items-center gap-2 rounded-xl bg-[#1f5c66] px-3 py-2.5 text-xs font-bold text-white shadow-lg shadow-[#1f5c66]/15 transition hover:bg-[#164d57] active:scale-[.98]"><Plus className="h-4 w-4" /> <span className="hidden sm:inline">إضافة موعد</span></button><button aria-label="الإشعارات" className="flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500"><span className="relative"><Radio className="h-4 w-4" /><span className="absolute -left-1 -top-1 h-2 w-2 rounded-full bg-orange-400 ring-2 ring-white" /></span></button></div>
        </header>
